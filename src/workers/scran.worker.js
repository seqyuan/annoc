...
function createDataset(args) {
  if (args.format === "10X") {
    ...
    // For AnalysisMode, use H5adDataset to allow analysis
    // For ExplorerMode, use H5adResult to load pre-computed results
    // The mode is determined by the caller (NewAnalysis component)
    if (args.mode === "analysis") {
      return new bakana.H5adDataset(args.h5, args.options ? args.options : {});
    } else {
      // Default to H5adResult for explore mode
